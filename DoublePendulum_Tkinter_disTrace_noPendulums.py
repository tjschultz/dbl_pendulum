...
# constants
G = 9.81 # m/s^2
N_COMP_PENDULUMS = 10 # set number of double pendulums to animate
...
class App(tk.Tk):
    ...
    pendulums1 = []
    pendulums2 = []
    trace_width = 200 # n of iterations before trace disappears
    iterations = 0
    
    def __init__(self,
                 pendulums1: [], pendulums2: [], trace_coords: [], colors: [],
                 width: int = 600,
    ...
    def draw_pendulums(self, index):
        ...
        self.trace_coords[index].append(
            (
        ...
        # draw the trace
        if (self.iterations <= self.trace_width):
            self.canvas.create_line(self.trace_coords[index], fill=self.colors[index], tag='trace', width=3)
        else:
            self.canvas.create_line(self.trunc(self.trace_coords[index]), fill=self.colors[index], tag='trace', width=3)
        ...
        self.canvas.create_line(
            ...
            width=p1.width, fill=self.colors[index], tags='pendulum'
        )
        ...
        self.canvas.create_line(
            ...
            width=p2.width, fill=self.colors[index], tags='pendulum'
        )
        """

    def trunc(self, tarr):
        return tarr[self.iterations-self.trace_width:]
...
if __name__ == '__main__':
    ...
    for i in range(N_COMP_PENDULUMS):
        ...
        theta1 = (1.5 *m.pi) + 1/100000*i
        theta2 = (1.9 *m.pi) + 1/100000*i
        ...
        int1 = int(random.randrange(255))
        int2 = int(random.randrange(255))
        int3 = int(random.randrange(255))
        
        colors.append('#%02x%02x%02x' % (int1, int2, int3))
